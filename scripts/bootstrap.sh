...
Examples:
  bash install.sh --repo https://github.com/your-org/paypanel-alipay.git
  bash install.sh --repo https://github.com/your-org/paypanel-alipay.git --mode aapanel --domain pay.example.com
USAGE
}
...
if [ "$(id -u)" -ne 0 ]; then
  fail "请使用 root 执行，例如：sudo bash install.sh --repo <git-url>"
fi

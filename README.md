...
- **多账户接入**：可配置多个支付宝开放平台应用，支持按失败次数轮询，请求失败自动切换下一个可用账户，支付宝密钥参数会按业务明文存入 SQLite，便于直接查看和排错。
...
- 支付宝 APPID、应用公钥、应用私钥、支付宝公钥等账户参数均按业务明文存入 SQLite，不再加密或隐藏；请务必保护服务器、数据库和备份文件权限。
- 顶部“日志”页面记录登录、设置、账户、订单、支付宝通知以及 HTTP 请求等操作/请求日志，并支持搜索和一键清空。
- 订单页支持按创建时间范围清理订单，或一键清除所有订单记录；订单列表和详情页也支持单个订单删除。
...
- 开启 2FA 前先在设置页生成 TOTP 密钥并用认证器保存；保存开启 2FA 时必须输入正确动态验证码，否则不会启用。
